...
class SpamRdfDatastream < ActiveFedora::NtriplesRDFDatastream
  ...
  map_predicates do |map|
    ...
    map.affiliation(to: 'affiliation', in: RDF::SCHEMA) do |index|
      index.as :stored_searchable, :facetable
    end

    map.organization(to: 'Organization', in: RDF::SCHEMA) do |index|
      index.as :stored_searchable, :facetable
    end

    map.administrative_unit(to: 'department', in: RDF::SCHEMA) do |index|
      index.as :stored_searchable, :facetable
    end
