...
describe 'display thumbnail' do
  ...
  context 'Display thumbnail for associated iamges' do
    ...
    it 'shows thumbnail for all related files' do
      ...
      page.should have_css("img[src$='#{generic_file2.to_param}/thumbnail'][class$='thumbnail']")
    end
  end
  ...
  context 'File Representative:' do
    ...
    it 'should show thumbnail when a representative is selected for the work' do
      login_as(user)
      visit curation_concern_image_path(curation_concern)
      page.should have_css("img[src$='#{generic_file1.to_param}/thumbnail'][class$='representative_image']")
      page.should have_css("img[src$='#{generic_file1.to_param}/thumbnail'][class$='thumbnail']")
      page.should have_css("img[src$='#{generic_file2.to_param}/thumbnail'][class$='thumbnail']")
      page.should_not have_css("img[src$='#{generic_file2.to_param}/thumbnail'][class$='representative_image']")
    end
  end
end
